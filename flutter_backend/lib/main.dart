...
void _startServer() async {
  ...
  // 获取初始路径
  router.get('/api/initial-paths', (Request request) async {
    final directory = await getApplicationDocumentsDirectory();
    final home = directory.path;
    final downloads = p.join(home, 'Downloads');
    final desktop = p.join(home, 'Desktop');
    final configDir = p.join(home, 'config');
  ...
  // 列出PDF文件
  router.get('/api/list-pdfs', (Request request) {
    final path = request.url.queryParameters['path'] ?? '';
    final dir = Directory(path);
    if (!dir.existsSync()) {
      return Response.badRequest(body: jsonEncode({'error': '目录不存在'}));
    }
    
    final pdfs = dir.listSync()
        .whereType<File>()
    ...
    return Response.ok(jsonEncode(pdfs), headers: {'Content-Type': 'application/json'});
  });
